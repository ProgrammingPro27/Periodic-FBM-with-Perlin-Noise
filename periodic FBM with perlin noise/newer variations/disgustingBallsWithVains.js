...
function calculateWorleyNoise(x, y) {
    ...
    return map(sorted[0], 0, 35, 255, 0)
}
...
function pattern(x, y, octaves, lacunarity, gain) {
    ...
    return fbm(x + 60.0 * q[0], y + 60.0 * q[1], octaves, lacunarity, gain);
};
...
function combinedNoise(x, y) {
    ...
    const worleyWeight = 0.5;
    const fbmWeight = 0.4;

    const worleyValue = calculateWorleyNoise(x, y);

    const fbmValue = 255 - ridge(pattern(x, y, octaves, lacunarity, gain)) * 255;
